...
export default function Sidebar({ onSelectMaterial }: SidebarProps) {
    const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>({
          'Aluminyum Alasimlari': true,
          'Paslanmaz Celikler': true,
          'Bakir Alasimlari': true,
          'Polimerler': true,
    });
    ...
    return (
          <div className={styles.sidebar}>
                  <div className={styles.sidebarHeader}>
                            ...
                            <h2 className={styles.headerTitle}>Malzeme Kutuphanesi</h2>h2>
                  </div>div>
                      ...
                      <div key={category.name} className={styles.categoryWrapper}>
                                    <div 
                                      ...
                                      {expandedCategories[category.name] ? (
                                                  ...
                                                  <span className={styles.categoryName}>{category.name}</span>span>
                                    </div>div>
                                        ...
                                        {category.materials.map((material) => (
                                                          <div 
                                                                              ...
                                                                              <div className={styles.materialInfo}>
                                                                                                    <span className={styles.materialName}>{material.name}</span>span>
                                                                              </div>div>
                                                          </div>div>
                                                        ))}
                                      </div>div>
